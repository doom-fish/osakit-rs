# osakit-rs coverage audit (vs MacOSX26.2.sdk)

Audit source: `OSAKit.framework/Headers/*.h` from `/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX26.2.sdk`.

Counts below treat each public Objective-C `@interface`, typedef, `extern const`, method, and property as a distinct SDK symbol. The tables collapse a few repetitive constant/property families into single audit rows where the crate exposes them through one safe Rust wrapper.
...
| `-initWithComponent:` | initializer | `OSALanguage.h` | `OsaComponent::language()` round-trip, paired with `Language::component()` |
